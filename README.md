...
- на iPad **один палец по картинке выбирает и двигает именно картинку**, при этом активный Pen не переключается и Apple Pencil продолжает писать;
- tap пальцем по пустому canvas закрывает редактирование картинки и сразу возвращает обычную навигацию; отдельного «режима выхода» нет;
- если первый палец держит картинку и появляется второй, текущий object-drag отменяется и оба пальца сразу переходят в pinch/pan;
...
- рамка выделения появляется **только после реального drag** (6 px для мыши, 10 px для touch), поэтому простой клик и микродрожание курсора больше не создают синее окно;
- **ПКМ + удержание + drag** — временная рамка выделения из любого инструмента; простой ПКМ-клик ничего не меняет, отпускание после drag завершает выбор;
- `Ctrl/Cmd + drag` временно включает выделение даже из другого инструмента;
- выбранный объект начинает двигаться только после небольшого drag-threshold, поэтому случайный click/jitter его не сдвигает;
...
- вставка картинки выбирает её и показывает object controls, **но не переключает текущий Pen/Eraser/Pan tool**;
- при выборе одной картинки появляется **контекстная мини-панель прямо возле неё**, а image-specific действия убраны из глобального toolbar;
- в контекстной панели картинки: **Обрезать, Копировать, Дубликат, назад/вперёд по слоям изображений, Сбросить crop, Удалить**;
- `Ctrl/Cmd+C` + `Ctrl/Cmd+V` работает для копии выбранной картинки внутри текущей доски, `Ctrl/Cmd+D` сразу создаёт дубликат;
- **неразрушающее кадрирование изображений**: выбрать картинку → Обрезать (или double-click) → двигать рамку/8 handles → Готово/Отмена; crop metadata синхронизируется и сохраняется в snapshot;
- в Crop палец управляет crop-рамкой; Pencil при начале письма завершает crop-сессию и снова пишет;
- порядок изображений (`front/back`) синхронизируется realtime и сохраняется в snapshot;
...
- **вставка и удаление картинок тоже входят в Undo/Redo**: красный `×` → `↶` возвращает фото с размером/crop, `↷` снова удаляет;
...
Главная `/` является списком досок. Создай новую доску или открой прошлую карточкой. Внутри комнаты кнопка **Поделиться** копирует `/b/<4-digit-code>`.

> 4-значный код удобен для занятия, но не является авторизацией. Перед постоянным публичным интернет-деплоем нужен отдельный owner/invite credential — см. `docs/deployment.md`.
...
| Редактировать картинку на iPad | tap пальцем по картинке → меню; drag этим же пальцем → перемещение |
| Выйти из редактирования картинки | tap пальцем по пустому canvas |
...
| Zoom | два пальца / колесо; второй палец также прерывает текущий drag картинки и начинает pinch |
| Ластик | `E`; Pencil/мышь, а с direct ink — также один палец |
| Select | `V` |
| Рамка выделения | drag по пустому месту; появляется только после drag-threshold |
| Быстрая рамка выделения | удерживать `ПКМ` и тянуть; простой ПКМ-клик ничего не делает |
...
| Действия с картинкой | выбрать картинку → контекстная панель возле изображения |
| Кадрировать изображение | Обрезать в контекстной панели или double-click → рамка/handles → Готово/Отмена |
| Копировать картинку | `Ctrl/Cmd+C`, затем `Ctrl/Cmd+V` |
| Дублировать картинку | контекстная кнопка или `Ctrl/Cmd+D` |
| Undo/Redo | `Ctrl/Cmd+Z`, `Ctrl/Cmd+Shift+Z`; включает рисование, стирание, вставку и удаление изображений |
