...
Главная `/` создаёт комнату, после чего преподаватель отправляет ученику ссылку `/b/<room-id>`.
...
## Room IDs and security

Текущий случайный room id имеет высокую энтропию и удобен как invitation UX, но **«кто знает ссылку, тот имеет доступ» не является полноценной системой безопасности**.

Для реальных занятий возможны два продуктовых варианта:

- teacher account + invite token для ученика;
- room owner secret + participant invite token без обязательных student accounts.

Выбор стоит делать позже вместе с UX занятий. Не надо сейчас встраивать сложную auth-систему в realtime ink core.
